...
if DEBATE_MODE:
    CHECK_INTERVAL = 120    # 2 minutes per cycle (120 seconds)
    POST_RECENT_MINUTES = 60  # increased to 60 minutes to find posts for testing
    PROCESSING_LIMIT = 30   # maximum processing during peak debate time
    DELAY_BETWEEN_POSTS = 1.0  # faster response during live broadcast
    DELAY_BETWEEN_SEARCHES = 0.5  # quick searches during debate
    DELAY_AFTER_LOGIN = 0.3
    DELAY_AFTER_API_CALL = 0.2  # minimal delay for live responsiveness
    ENGAGEMENT_THRESHOLD = 0  # engage with ALL posts during live debate
else:
    CHECK_INTERVAL = 120    # 2 minutes for normal mode too
...
def run_bot():
    ...
    try:
        print("🔐 Logging in...")
        safe_api_call(t.login, "alex.martin", "vR8!k2Lq#Z1p7d?")
        ...
        # Generate single post per cycle for all modes (2-minute intervals)
        posts_to_create = 1  # One post every 2 minutes for all modes
...
if __name__ == "__main__":
    ...
    print("⏰ STRATEGIC MODE - 2 minute intervals, 1 post per cycle")
    print("💪 Ready to convert undecided voters with trending hashtags!")
    print("=" * 60)
