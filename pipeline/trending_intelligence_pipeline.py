...
class TrendingIntelligencePipeline:
    ...
    async def execute_full_pipeline(self, **kwargs) -> Dict[str, Any]:
        ...
        logger.info(f"🚀 Starting Trending Intelligence Pipeline - Batch: {batch_id}")
        ...
        try:
            ...
            pipeline_results = await asyncio.wait_for(
                self._execute_pipeline_stages(batch_id, stage_results, pipeline_summary),
            ...
            logger.info(f"✅ Pipeline completed - Batch: {batch_id}")
            logger.info(f"📊 Success rate: {pipeline_summary['success_rate']:.1%}")
            logger.info(f"⏱️  Total execution time: {total_execution_time:.1f}s")
            
            return pipeline_summary

        except asyncio.TimeoutError:
            logger.error(f"❌ Pipeline timeout after {self.pipeline_timeout}s - Batch: {batch_id}")
            pipeline_summary.update({
                "status": "timeout",
            ...
            logger.error(f"❌ Pipeline error - Batch: {batch_id}: {e}")
        ...
        for stage_num in range(1, 11):  # Stages 1-10
            ...
            try:
                ...
                if stage_result:
                    ...
                    logger.info(f"✅ Stage {stage_num}: {stage_name} - Completed ({execution_time:.1f}s)")
                    
                else:
                    # Stage failed but pipeline can continue
                    logger.warning(f"⚠️ Stage {stage_num}: {stage_name} - Failed but continuing pipeline")
                ...
                logger.error(f"❌ Stage {stage_num}: {stage_name} - Timeout after {self.stage_timeout}s")
                pipeline_summary["failed_stages"].append(stage_num)
                pipeline_summary["stage_results"][stage_num] = {
                    "stage_name": stage_name,
                    "status": "timeout",
    ...
    def _create_stage_summary(self, stage_result: Dict) -> Dict:
        ...
        if "output_path" in stage_result:
            summary["output_file"] = os.path.basename(stage_result["output_path"])
    ...
    async def execute_partial_pipeline(self, start_stage: int = 1, end_stage: int = 10, **kwargs) -> Dict[str, Any]:
        ...
        logger.info(f"🔄 Starting Partial Pipeline (Stages {start_stage}-{end_stage}) - Batch: {batch_id}")
        
        if start_stage < 1 or end_stage > 10 or start_stage > end_stage:
            raise ValueError("Invalid stage range. Must be 1 <= start_stage <= end_stage <= 10")
        ...
        try:
            ...
            for stage_num in range(start_stage, end_stage + 1):
                ...
                try:
                    stage_result = await asyncio.wait_for(
                        self._execute_single_stage(stage_num, stage_instance, batch_id, previous_stage_data),
                        timeout=self.stage_timeout
                    )
                    ...
                    if stage_result:
                        ...
                        logger.warning(f"⚠️ Stage {stage_num}: {stage_name} - Failed")
                        pipeline_summary["failed_stages"].append(stage_num)
                        pipeline_summary["stage_results"][stage_num] = {
                            "stage_name": stage_name,
                            "status": "failed",
            ...
            logger.info(f"✅ Partial Pipeline completed - Batch: {batch_id}")
            logger.info(f"📊 Success rate: {pipeline_summary['success_rate']:.1%}")
            
            return pipeline_summary

        except Exception as e:
            logger.error(f"❌ Partial Pipeline error - Batch: {batch_id}: {e}")
            pipeline_summary.update({
                "status": "error",
                "error": str(e),
                "completed_at": datetime.now(timezone.utc).isoformat()
    ...
    async def _load_previous_stage_data(self, batch_id: str, stage_num: int) -> Optional[Dict]:
        ...
        logger.info(f"📁 Attempting to load previous data from stage {stage_num}")
        return None
    ...
    async def validate_pipeline(self) -> Dict[str, Any]:
        ...
        for stage_num, stage_instance in self.stages.items():
            ...
            try:
                ...
                if not hasattr(stage_instance, 'execute'):
                    validation_results["issues"].append(f"Stage {stage_num} ({stage_name}) missing execute method")
                    validation_results["stage_validation"][stage_num] = "invalid"
                else:
                    validation_results["stage_validation"][stage_num] = "valid"
                    
            except Exception as e:
                validation_results["issues"].append(f"Stage {stage_num} ({stage_name}) validation error: {e}")
        ...
        for dir_path in required_dirs:
            if not os.path.exists(dir_path):
                validation_results["warnings"].append(f"Directory {dir_path} does not exist - will be created during execution")
