...
class ReportingStage:
    ...
    async def execute(self, batch_id: str, prediction_data: Dict, **kwargs) -> Optional[Dict]:
        ...
        logger.info("🔔 Stage 8: Generating intelligence report...")
        ...
        try:
            ...
            logger.info(f"✅ Stage 8 completed: Intelligence report generated")
            logger.info(f"📊 Report includes {len(insights.get('top_posts', []))} highlighted posts")
    ...
    async def _generate_comprehensive_summary(self, posts: List[Dict], batch_id: str) -> Dict:
        ...
        classifications = [post.get("classification", "Unknown") for post in posts]
    ...
    async def _create_insights_and_highlights(self, posts: List[Dict]) -> Dict:
        ...
        top_velocity_posts = sorted(posts, key=lambda p: p.get("velocity_per_min", 0), reverse=True)[:5]
        top_rubric_posts = sorted(posts, key=lambda p: p.get("final_score", 0), reverse=True)[:5]
        top_prediction_posts = sorted(posts, key=lambda p: p.get("trending_probability", 0), reverse=True)[:5]
        top_engagement_posts = sorted(posts, key=lambda p: p.get("total_engagement", 0), reverse=True)[:5]
        ...
        if high_performers:
            ...
            avg_length = sum(p.get("content_length", 0) for p in high_performers) / len(high_performers)
            avg_tags = sum(p.get("tag_count", 0) for p in high_performers) / len(high_performers)
        ...
        viral_candidates = [p for p in posts if p.get("trending_probability", 0) >= 0.8]
        if viral_candidates:
            common_factors = []
            ...
            top_factors = [factor for factor, count in factor_counts.most_common(3)]
        ...
        high_engagement_posts = [p for p in posts if p.get("total_engagement", 0) > 20]
        if high_engagement_posts:
            avg_reply_ratio = sum(p.get("reply_count", 0) / max(1, p.get("total_engagement", 1)) for p in high_engagement_posts) / len(high_engagement_posts)
    ...
    def _extract_key_findings(self, posts: List[Dict]) -> List[str]:
        ...
        high_score_count = len([p for p in posts if p.get("final_score", 0) >= 70])
        if high_score_count > 0:
            percentage = (high_score_count / len(posts)) * 100
            findings.append(f"{percentage:.1f}% of posts scored 70+ on viral rubric")
        
        # Trending potential
        high_prob_count = len([p for p in posts if p.get("trending_probability", 0) >= 0.6])
        if high_prob_count > 0:
            percentage = (high_prob_count / len(posts)) * 100
            findings.append(f"{percentage:.1f}% of posts show strong trending potential")
        ...
        if velocities:
            ...
            if max_velocity > 5:
                findings.append(f"Peak engagement velocity reached {max_velocity:.1f} interactions/minute")
        ...
        if tag_posts and no_tag_posts:
            tag_avg_score = sum(p.get("final_score", 0) for p in tag_posts) / len(tag_posts)
            no_tag_avg_score = sum(p.get("final_score", 0) for p in no_tag_posts) / len(no_tag_posts)
            
            if tag_avg_score > no_tag_avg_score + 5:
                findings.append("Posts with hashtags significantly outperform those without")
    ...
    async def _generate_llm_analysis(self, summary: Dict, insights: Dict) -> Dict:
        ...
        try:
            ...
            strategy_response = await self.llm.call_openai(
                strategy_prompt,
            ...
            trend_response = await self.llm.call_openai(
                trend_prompt,
...
**Viral Candidates:** {summary.get('viral_candidates', 0)}""",
        ...
        if strategy_insights:
            insight_text = "\n".join([f"• {insight.get('insight', '')}" for insight in strategy_insights[:2]])
            embed["fields"].append({
                "name": "💡 Key Insights",
                "value": insight_text,
                "inline": False
        ...
        if llm_analysis.get('strategic_recommendations'):
            recommendations = llm_analysis['strategic_recommendations'][:500] + "..." if len(llm_analysis['strategic_recommendations']) > 500 else llm_analysis['strategic_recommendations']
            embed["fields"].append({
    ...
    async def _create_text_report(self, summary: Dict, insights: Dict, llm_analysis: Dict, batch_id: str) -> str:
        ...
        report_lines = [
            f"🎯 TRENDING INTELLIGENCE REPORT - {batch_id}",
            ...
            "",
            "🏷️ TOP TRENDING TAGS:",
            " ".join([f"#{tag}" for tag in summary.get('trending_tags', [])[:8]]),
            "",
            "🎯 PERFORMANCE DISTRIBUTION:"
        ]
