...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('debug_scheduler_clean.log')
    ]
)
...
async def debug_worker_and_scheduler():
    ...
    try:
        ...
        config = get_config()
        logger.info(
            f"[OK] Config loaded: interval = {config.collection_interval_minutes} minutes")

        # Initialize worker
        logger.info("Step 4: Initializing worker...")
        worker = BackgroundWorker()
        logger.info(f"[OK] Worker initialized: {worker}")
        ...
        if hasattr(worker, '_run_trending_intelligence_task_with_cleanup'):
            logger.info(
                "[OK] _run_trending_intelligence_task_with_cleanup method exists")
        else:
            logger.error(
                "[ERROR] _run_trending_intelligence_task_with_cleanup method NOT found")

        if hasattr(worker, '_run_trending_intelligence_task'):
            logger.info("[OK] _run_trending_intelligence_task method exists")
        else:
            logger.error(
        ...
        try:
            ...
            if method and callable(method):
                logger.info("[OK] Cleanup method is callable")
                logger.info("   Method signature: {}".format(
                    method.__doc__ or "No docstring"))
            else:
                logger.error("[ERROR] Cleanup method is not callable")
        except Exception as e:
            logger.error(f"[ERROR] Error checking cleanup method: {e}")
        ...
        logger.info(f"[OK] Scheduler setup successful with {len(jobs)} jobs")
        for job in jobs:
            # Use trigger.interval instead of next_run_time
            interval_info = f"interval: {job.trigger.interval}" if hasattr(
                job.trigger, 'interval') else "no interval info"
            logger.info(
                f"   - Job: {job.id}, func: {job.func.__name__}, {interval_info}")
        ...
        try:
            ...
            logger.info(
                "[OK] trending_intelligence_task imported successfully")

            if hasattr(trending_intelligence_task, 'run'):
                logger.info(
                    "[OK] trending_intelligence_task.run method exists")
            else:
                logger.error(
                    "[ERROR] trending_intelligence_task.run method NOT found")
        except ImportError as e:
            logger.error(
                f"[ERROR] Failed to import trending_intelligence_task: {e}")
...
async def test_scheduler_timing():
    ...
    try:
        ...
        logger.error(f"[ERROR] Timing test failed: {e}")
...
async def main():
    ...
    logger.info("Check debug_scheduler_clean.log for detailed output")
