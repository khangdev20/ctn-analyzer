...
def fetch_all_pages(api_url: str, timeout: int = 15, max_retries: int = 3, max_pages: int = 2) -> Tuple[List[Dict], str]:
    """
    Fetch limited pages from leaderboard API using pagination
    ...
    try:
        while True:
            page_count += 1
            logger.info(f"[ANALYTICS] Fetching leaderboard page {page_count} (cursor: {next_cursor})")
            ...
            try:
                ...
                if not page_rows:
                    logger.warning(f"[WARNING] Page {page_count} returned no data")
                    break
                
                all_rows.extend(page_rows)
                logger.info(f"[OK] Page {page_count}: Got {len(page_rows)} rows (total: {len(all_rows)})")
                ...
                if page_count >= max_pages:
                    logger.info(f"[LIMIT] Reached page limit ({max_pages}). Total pages: {page_count}, Total rows: {len(all_rows)}")
                    status = "partial" if has_next and next_cursor else "complete"
                    break
                
                if not has_next or not next_cursor:
                    logger.info(f"[COMPLETE] Pagination complete. Total pages: {page_count}, Total rows: {len(all_rows)}")
                    break
                    
            except requests.exceptions.RequestException as e:
                logger.error(f"[HOT] Request failed on page {page_count}: {e}")
    ...
    logger.info(f"[REPORT] Fetch complete: {len(all_rows)} total rows, status: {status}")
...
def compare_snapshots(today: List[Dict], yesterday: Optional[List[Dict]]) -> Dict:
    ...
    for today_entry in today:
        ...
        if entry_id not in yesterday_by_id:
            ...
            rank_delta = yesterday_entry['rank'] - today_entry['rank']  # Positive = moved up
    ...
    # Dropouts: by yesterday rank (best former rank first)
    dropouts.sort(key=lambda x: x['yesterday_rank'])
    dropouts = dropouts[:5]
...
def format_bidaily_discord_message(timestamp: str, comp: Dict, source_status: str, time_context: str) -> str:
    ...
    if not comp.get('is_first_run', False):
        # 2. Top Movers Up (past 12 hours)
        movers_up = comp.get('movers_up', [])
        if movers_up:
            lines.append("[TRENDING_UP] **Top Movers ↑ (past 12h)**")
            for mover in movers_up:
                line = _format_mover_up_line(mover)
                lines.append(line)
            lines.append("")
        
        # 3. Top Movers Down (past 12 hours)
        movers_down = comp.get('movers_down', [])
        if movers_down:
            lines.append("[TRENDING_DOWN] **Top Movers ↓ (past 12h)**")
        ...
        if new_entries:
            lines.append("[SPARKLE] **New Entries (past 12h)**")
        ...
        if dropouts:
            lines.append("📴 **Dropouts (past 12h)**")
    ...
    total_previous = comp.get('total_yesterday', 0)
    ...
    if comp.get('is_first_run', False):
        ...
        lines.append(f"[ANALYTICS] **Total Entries:** {total_current:,} {change_str}")
    
    lines.append(f"⏱ **Generated:** {current_time} | **Source:** {status_emoji}")
...
def format_discord_message(date_local: str, comp: Dict, source_status: str) -> str:
    ...
    if not comp.get('is_first_run', False):
        ...
        movers_up = comp.get('movers_up', [])
        if movers_up:
            lines.append("[TRENDING_UP] **Top Movers ↑ (rank)**")
            for mover in movers_up:
                line = _format_mover_up_line(mover)
                lines.append(line)
            lines.append("")
        ...
        movers_down = comp.get('movers_down', [])
        if movers_down:
            lines.append("[TRENDING_DOWN] **Top Movers ↓ (rank)**")
            for mover in movers_down:
                line = _format_mover_down_line(mover)
                lines.append(line)
            lines.append("")
        ...
        new_entries = comp.get('new_entries', [])
        if new_entries:
            lines.append("[SPARKLE] **New Entries**")
            for entry in new_entries:
                line = _format_new_entry_line(entry)
                lines.append(line)
            lines.append("")
    ...
    total_yesterday = comp.get('total_yesterday', 0)
    ...
    if comp.get('is_first_run', False):
        ...
        change_str = f"({change:+,})" if change != 0 else ""
        lines.append(f"[ANALYTICS] **Total Entries:** {total_today:,} {change_str}")
...
def _format_dropout_line(dropout: Dict) -> str:
    ...
    yesterday_rank = dropout['yesterday_rank']
    yesterday_score = dropout['yesterday_score']
    
    return f"{name} (#{yesterday_rank}) — prev {_format_score(yesterday_score)}"
...
def _truncate_message(lines: List[str]) -> str:
    ...
    for section_header, section_name in sections_to_try:
        ...
        try:
            ...
            logger.warning(f"[WARNING] Error truncating section {section_name}: {e}")
            continue
...
def send_discord(message: str, webhook_url: str) -> Dict:
    ...
    try:
        if not webhook_url:
            logger.warning("[WARNING] No Discord webhook URL provided, skipping Discord send")
            return {'success': False, 'error': 'No webhook URL configured'}
            
        # Log webhook type based on URL pattern
        if 'leaderboard' in webhook_url.lower() or '1425285300183109644' in webhook_url:
            logger.info("[WEBHOOK] Sending leaderboard report via DEDICATED leaderboard webhook")
        else:
            logger.info("[WEBHOOK] Sending leaderboard report via FALLBACK main webhook")
...
async def leaderboard_bidaily_task(force_post: bool = False) -> Dict:
    ...
    try:
        ...
        logger.info(f"[TIMER] Processing bi-daily leaderboard for {current_timestamp} (AEST)")
        
        # 3. Lock guard using Redis (unless forced)
        lock_key = f"leaderboard:lock:{current_timestamp}"
        if not force_post:
            if redis_client.exists(lock_key):
                logger.info(f"[LOCKED] Already processed {current_timestamp} - skipping (use force_post=True to override)")
                return {
                    'status': 'skipped',
        ...
        logger.info("[ANALYTICS] Fetching current leaderboard data (first 2 pages only)...")
        raw_data, source_status = fetch_all_pages(api_url, timeout, max_retries, max_pages=2)
        ...
        redis_client.setex(current_key, 7 * 24 * 3600, json.dumps(current_entries))  # Keep for 7 days
        
        # 7. Get previous snapshot from Redis (12 hours ago)
        logger.info("📖 Loading previous snapshot from Redis...")
        previous_12h = local_now - timedelta(hours=12)
        previous_timestamp = previous_12h.strftime("%Y-%m-%d_%H-%M")
        previous_key = f"leaderboard:snapshot:{previous_timestamp}"
        
        previous_entries = None
        previous_data = redis_client.get(previous_key)
        if previous_data:
            try:
                previous_entries = json.loads(previous_data)
                logger.info(f"[OK] Loaded previous snapshot: {len(previous_entries)} entries")
            except json.JSONDecodeError as e:
                logger.warning(f"[WARNING] Failed to decode previous snapshot: {e}")
        else:
            # Fallback: try to find any snapshot from same day
            pattern = f"leaderboard:snapshot:{today_str}_*"
            existing_keys = redis_client.keys(pattern)
            if existing_keys:
                # Get the most recent one
                existing_keys.sort(reverse=True)
                fallback_data = redis_client.get(existing_keys[0])
                if fallback_data:
                    try:
                        previous_entries = json.loads(fallback_data)
                        logger.info(f"[OK] Using fallback snapshot: {len(previous_entries)} entries")
                    except json.JSONDecodeError:
                        logger.warning("[WARNING] Failed to decode fallback snapshot")
        ...
        message = format_bidaily_discord_message(current_timestamp, comparison, source_status, time_context)
        ...
        if not webhook_url:
            ...
            if webhook_url:
                logger.info("[FALLBACK] Using main Discord webhook for leaderboard")
            else:
                logger.warning("[WARNING] No Discord webhook URL configured (LEADERBOARD_DISCORD_WEBHOOK or DISCORD_WEBHOOK)")
        else:
            logger.info("[WEBHOOK] Using dedicated leaderboard Discord webhook")
        ...
        logger.info(f"[OK] Bi-daily leaderboard task completed successfully for {current_timestamp}")
        return result
    ...
    try:
        ...
        if not force_post:
            ...
            if not lock_acquired:
                logger.info(f"[LOCKED] Already processed {today_str} - skipping (use force_post=True to override)")
                return {
                    'status': 'skipped',
                    'reason': 'already_processed',
                    'date': today_str
        ...
        # 6. Read yesterday's snapshot
        logger.info("📖 Loading yesterday's snapshot...")
        yesterday_entries = read_snapshot(yesterday_str)
        
        if yesterday_entries is None:
            logger.info("🆕 No yesterday snapshot found - first run mode")
        else:
            logger.info(f"[OK] Loaded yesterday snapshot: {len(yesterday_entries)} entries")
        ...
        result = {
            ...
            'entries_yesterday': len(yesterday_entries) if yesterday_entries else 0,
            'source_status': source_status,
            'discord_sent': discord_result.get('success', False),
            'message_length': len(message),
            'comparison': {
        ...
        logger.info(f"[OK] Daily leaderboard task completed successfully for {today_str}")
        return result
