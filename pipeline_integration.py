...
class PipelineIntegration:
    ...
    async def execute_trending_intelligence_task(self, worker=None) -> Dict[str, Any]:
        ...
        logger.info(f"🚀 Starting Trending Intelligence Task: {task_id}")
        ...
        try:
            ...
            logger.info(f"✅ Trending Intelligence Task completed: {task_id}")
            logger.info(f"📊 Pipeline success rate: {pipeline_result.get('success_rate', 0):.1%}")
            
            return task_result

        except Exception as e:
            logger.error(f"❌ Trending Intelligence Task failed: {task_id} - {e}")
            return {
                "task_id": task_id,
                "status": "error",
                "error": str(e),
    ...
    async def execute_partial_pipeline(self, start_stage: int = 1, end_stage: int = 10) -> Dict[str, Any]:
        ...
        logger.info(f"🔄 Starting Partial Pipeline (Stages {start_stage}-{end_stage}): {task_id}")
        
        try:
            pipeline_result = await self.pipeline.execute_partial_pipeline(start_stage, end_stage)
            task_result = await self._format_task_result(pipeline_result, task_id)
            
            logger.info(f"✅ Partial Pipeline completed: {task_id}")
            return task_result

        except Exception as e:
            logger.error(f"❌ Partial Pipeline failed: {task_id} - {e}")
            return {
                "task_id": task_id,
                "status": "error",
                "error": str(e),
    ...
    async def validate_pipeline_setup(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if validation_result["overall_status"] == "valid":
                logger.info("✅ Pipeline validation successful - system ready")
            else:
                logger.warning(f"⚠️ Pipeline validation issues detected: {len(validation_result['issues'])} issues")
