...
class TestTemporalAnalytics(unittest.TestCase):
    ...
    def test_temporal_analytics_core(self):
        ...
        async def run_test():
            ...
            print(
                f"✅ Analyzed {summary['total_posts']} posts over {summary['time_span_hours']:.1f} hours")
    ...
    def test_hourly_pattern_analysis(self):
        ...
        async def run_test():
            ...
            print(
                f"✅ Found {len(hourly_patterns['best_hours'])} optimal hours")
            print(
                f"✅ Detected {len(hourly_patterns['optimal_ranges'])} time ranges")
    ...
    def test_daily_pattern_analysis(self):
        ...
        async def run_test():
            ...
            print(
                f"✅ Best days: {', '.join(daily_patterns['best_day_names'][:3])}")
    ...
    def test_time_to_trend_analysis(self):
        ...
        async def run_test():
            ...
            if trend_analysis["trending_posts_count"] > 0:
                self.assertGreater(
                    trend_analysis["avg_time_to_trend_minutes"], 0)
                # Less than 10 hours
                self.assertLess(
                    trend_analysis["avg_time_to_trend_minutes"], 600)

            print(
                f"✅ Analyzed {trend_analysis['trending_posts_count']} trending posts")
            if trend_analysis["avg_time_to_trend_minutes"] > 0:
                print(
                    f"✅ Average time to trend: {trend_analysis['avg_time_to_trend_minutes']:.1f} minutes")
    ...
    def test_momentum_duration_analysis(self):
        ...
        async def run_test():
            ...
            if momentum_analysis["analyzed_posts"] > 0:
                ...
                # Less than 2 days
                self.assertLess(
                    momentum_analysis["avg_momentum_duration_hours"], 48)

            print(
                f"✅ Analyzed momentum for {momentum_analysis['analyzed_posts']} posts")
            if momentum_analysis["avg_momentum_duration_hours"] > 0:
                print(
                    f"✅ Average momentum duration: {momentum_analysis['avg_momentum_duration_hours']:.1f} hours")
...
async def demo_temporal_analytics():
    ...
    print(
        f"• Time span: {temporal_summary.get('time_span_hours', 0):.1f} hours")
    print(
        f"• Avg engagement/hour: {temporal_summary.get('avg_engagement_per_hour', 0):.1f}")
    ...
    if trend_analysis.get("avg_time_to_trend_minutes", 0) > 0:
        print(
            f"• Avg time to trend: {trend_analysis['avg_time_to_trend_minutes']:.0f} minutes")

    momentum_analysis = result.get("momentum_analysis", {})
    if momentum_analysis.get("avg_momentum_duration_hours", 0) > 0:
        print(
            f"• Avg momentum duration: {momentum_analysis['avg_momentum_duration_hours']:.1f} hours")
